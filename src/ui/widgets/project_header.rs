...
pub struct ProjectHeaderWidget<'a> {
    pub project_uuid: &'a ThingsId,
    pub title: &'a str,
    pub id_prefix_len: usize,
}

impl<'a> ProjectHeaderWidget<'a> {
    fn bold() -> Style {
        Style::default().add_modifier(Modifier::BOLD)
    }
}

impl<'a> Widget for ProjectHeaderWidget<'a> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        render_id_prefix(self.project_uuid, self.id_prefix_len, id_col, buf);
